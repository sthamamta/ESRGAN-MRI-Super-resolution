...
def main():
    ...
    wandb.watch(generator,log="all",log_freq=1)
    ...
    for epoch in range(start_epoch, config.epochs):
        train(discriminator,
              ...
              scaler,
              wandb)
        _, _ = validate(generator, valid_prefetcher, epoch, wandb, psnr_model, ssim_model, "Valid")
        psnr, ssim = validate(generator, test_prefetcher, epoch, wandb, psnr_model, ssim_model, "Test")
        print("\n")
...
def train(discriminator: nn.Module,
    ...
    for batch_index,batch_data in enumerate(train_prefetcher):
        ...
        if batch_index % config.print_frequency == 0:
            ...
            "Train/D(HR)_Probability": d_hr_probability.item(),
            "Train/D(SR)_Probability":d_sr_probability.item(),
            "Train/epoch": epoch,
            "Train/iter": iters
            })
